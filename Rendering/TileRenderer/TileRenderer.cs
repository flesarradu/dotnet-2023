...
namespace Mapster.Rendering
{
    public static class TileRenderer
    {
        public static BaseShape Tessellate(this MapFeatureData feature, ref BoundingBox boundingBox, ref PriorityQueue<BaseShape, int> shapes)
        {
            ...
            if (feature.Properties.Any(f => f.Key == MapPropertyType.Highway && MapFeature.HighwayTypes.Any(h=>h.StartsWith(f.Value))))
            {
            ...
            else if (feature.Properties.Any(f=> f.Key == MapPropertyType.Water) &&
            ...
            else if (feature.Properties.Any(f => f.Key == MapPropertyType.Railway))
            {
                var coordinates = feature.Coordinates;
            ...
            else if (feature.Properties.Any(f => f.Key == MapPropertyType.Natural) &&
            ...
            else if (feature.Properties.Any(f => f.Key == MapPropertyType.Boundary &&
                    f.Value.StartsWith("forest")))
            {
                var coordinates = feature.Coordinates;
            ...
            else if (feature.Properties.Any(f => f.Key == MapPropertyType.Landuse &&
                     f.Value.StartsWith("forest") || f.Value.StartsWith("orchard")))
            {
                var coordinates = feature.Coordinates;
                var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Forest);
                baseShape = geoFeature;
                ...
                feature.Properties.Any(p=> p.Key == MapPropertyType.Landuse 
                && (   p.Value.StartsWith("residential") 
                    ...
                    || p.Value.StartsWith("brownfield"))))
            {
                ...
                && feature.Properties.Any(p => p.Key == MapPropertyType.Landuse 
                && (   p.Value.StartsWith("farm") 
                    || p.Value.StartsWith("meadow") 
                    ...
                    || p.Value.StartsWith("allotments"))))
            {
                ...
                shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     feature.Properties.Any(p => p.Key == MapPropertyType.Landuse && (p.Value.StartsWith("reservoir") || p.Value.StartsWith("basin")))) 
            { 
                var coordinates = feature.Coordinates;
                ...
                shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     feature.Properties.Any(f => f.Key == MapPropertyType.Building))
            {
                var coordinates = feature.Coordinates;
                var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
                baseShape = geoFeature;
                shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     feature.Properties.Any(f => f.Key == MapPropertyType.Leisure))
            {
                var coordinates = feature.Coordinates;
                var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
                baseShape = geoFeature;
                shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     feature.Properties.Any(f => f.Key == MapPropertyType.Amenity))
            {
                var coordinates = feature.Coordinates;
                var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
                baseShape = geoFeature;
